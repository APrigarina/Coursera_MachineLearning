...
yVec = zeros(m,num_labels); %5000x10

for i = 1:m
    yVec(i,y(i)) = 1;
end

regTheta = (lambda/(2*m)) * (sum(sum(Theta1(:,2:end).^2)) + sum(sum(Theta2(:,2:end).^2)));

J = - 1/m * sum(sum(yVec .* log(h) + (1-yVec) .* log(1-h))) + regTheta;
...
for t = 1:m
	...
	%BP
	y_bin = ([1:num_labels]==y(t))'; %10x1

	delta3 = a3 - y_bin; %10x1

	delta2 = (Theta2' * delta3) .* [1; sigmoidGradient(z2)]; %26x10 * 10x1 .* 26x1 = 26x1
